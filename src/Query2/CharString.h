...
public:
	...
	Status StrAssign(string str);                      //复制str的内容
	int indexOf(string str);                           //定位str
	bool findString(string s);                             
	Status deleteString(string s);                 //删掉字符串

	int StrLength();                                   //返回长度
	Status Concat(CharString s1, CharString s2);       //连接s1, s2的内容并返回状态
	Status SubString(int pos, int len);               //截断得到s的从pos起的长len的字符     
...
public:
	...
	Status seekLink(CharString cstr);                                //查找
