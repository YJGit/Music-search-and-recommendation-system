                           ...
                           CharStringLink& cStrLink)
{
    ...
    //建set
    //去中文符号
    string ChineseWordDic;
    QFile inDic(":/dic/mark.txt");
    if(!inDic.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qDebug()<<"Can't open the file!"<<endl;
    }

    QTextStream txtInput(&inDic);
    QString lineStr;

    while(!txtInput.atEnd())
    {
        lineStr = txtInput.readLine();
        ChineseWordDic = lineStr.toStdString();
        QTextCodec *code = QTextCodec::codecForName("gb18030");
        if(code)
            ChineseWordDic = code ->fromUnicode(lineStr).data();

         cStr.deleteString(ChineseWordDic);
    }
    ...
    if(!cStr.StrAssign(str))
    {
        CharString cs;
        cs.StrAssign("");
        cStrLink.createLink(cs);
        return;
    }
...
void MusicInfo::dealMusicTitle()
{
    //处理歌名
    string str("");
    int index = 0, k = 0;
    string _s("（");
    if(title.findString(_s))
    {
        index = title.indexOf(_s);
        while(k != index)
        {
            str += title.ch[k];
            k++;
    ...
    {
        while(title.ch[k] != '(' && k < title.length)
        {
            str += title.ch[k];
            k++;
        }
