...
Status CharString::StrAssign(string str)
{
	if(ch)
	{
		delete[] ch;
...
Status CharString::Concat(CharString s1, CharString s2)
{
	if(ch)
	{
		delete[] ch;
...
Status CharString::SubString(int pos, int len)
{
...
Status CharString::deleteString(string s)           
{
	if(!ch)
		return ERROR;
	int pos = indexOf(s);
...
Status CharStringLink::seekLink(CharString cstr)
{
	LinkNode *p = head;
	while(p != NULL)
	{
